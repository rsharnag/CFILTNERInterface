...
xash_word = document.form1.word;
</SCRIPT>
</head>
<body>
<?

$transword=$_GET['transWord'];
$transword=mysql_real_escape_string($transword);
$transword = preg_replace('!\s+!', ' ', $transword);
if($transword=="" || $transword== " "){
	echo "<div class=\"alert alert-warning fade in\" style=\"width:100%;\">
		...
		$resultTranslate = mysql_query("SELECT * FROM dictx WHERE uw_head = '".$transword."'") or die(mysql_error());
		$countResult = mysql_num_rows($resultTranslate);
		if($countResult==0){
			$resultTranslate = mysql_query("SELECT * FROM dictx WHERE hw_uni_full = '".$transword."'") or die(mysql_error());
			$countResult = mysql_num_rows($resultTranslate);
		}
		if($countResult==0){
			print "<hr>";
			print "<font size=4>No Results of the given word <strong>'".$transword."'</strong> were found in the dictionary</font>";
			print "<hr>";
		}
		else{
		print "<hr>";
